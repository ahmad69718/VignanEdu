...
export default function ProfileDropdownComponent()
{
    ...
    return (<div className = {`gap-3 border-b  fixed z-30 ${lightdarkmodevalue === 1 ? 'bg-stone-900' : 'bg-white'} ${profilepagestatus === 1 ? 'block' : 'hidden'} right-4 top-14 border border-blue-500 rounded-md pt-5 pb-5 pr-10 pl-10 flex flex-col items-center justify-center ${lightdarkmodevalue=== 1 ? 'text-gray-400' : 'text-black'}`}>
        <button><img className={`max-h-24 max-w-24 pb-3 border-b ${lightdarkmodevalue === 1 ? 'border-white' : 'border-gray-400'}`} src="profile.png" /></button>
        <p>{userdata.email}</p>
        <p className="text-xl font-serif text-blue-500">{userdata.username}</p>
